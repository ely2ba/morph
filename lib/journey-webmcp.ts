  ...
  return [
    {
      ...
      description:
        'Read this Wayline page before composing or changing an interface. Returns site-owned itineraries, calculations, actions, the native component vocabulary, current ordered composition, revision, locks, saved and hidden journeys, comparison state, and any open calculation.',
      inputSchema: {
        type: 'object',
        properties: {},
        additionalProperties: false,
    ...
    },
    {
      ...
      description:
        'Transform the conventional results into a persistent Wayline decision view. Components can independently select facts and metrics, group journeys, set their own sort, and emphasize supported journeys or metrics. Use only the supported site-owned fields returned by read_page. Wayline calculates eligibility, arrival, door-to-door time, risk, ranking, and rendering; callers cannot provide formulas, HTML, URLs, or claims. Include base_revision if a view already exists.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description:
        'Recompose the active Wayline interface in place or update journey assumptions. Native components can be replaced, added, removed, moved, grouped, sorted and emphasized independently. configure_component accepts only the settings to change; do not repeat its component ID inside the patch. Call read_page first. Human locks, saved journeys, hidden journeys, and comparison choices remain protected unless their exact semantic operation is requested.',
      inputSchema: {
        type: 'object',
        properties: {
          base_revision: { type: 'integer', minimum: 0 },
              ...
              oneOf: [
                ...
                },
                {
                  ...
                  properties: {
                    operation: {
                      type: 'string',
                      enum: ['lock_control', 'unlock_control'],
                    },
                    control_id: {
                      type: 'string',
                      enum: [
