...
export type Product = {
  ...
  capacityKg: number;
  energyClass: string;
  energyKwhPer100: number;
  waterLitresPerCycle: number;
  noiseDb: number;
  noiseClass: string;
  spinSpeed: number;
  cycleMinutes: number;
  widthCm: number;
  heightCm: number;
  installedDepthCm: number;
  sideClearanceCm: number;
  topClearanceCm: number;
  rearClearanceCm: number;
...
export const catalog: Product[] = seeds.map((seed, index) => {
  ...
  return {
    ...
    spinSpeed,
    cycleMinutes,
    widthCm: one(cabinetWidth + sideClearanceCm * 2),
    heightCm: one(cabinetHeight + topClearanceCm),
    installedDepthCm: one(cabinetDepth + rearClearanceCm),
    sideClearanceCm,
    topClearanceCm,
    rearClearanceCm,
    warrantyYears,
    ...
    tags: [
      ...
      type,
      `${capacityKg}kg`,
      `energy-${energyClass.toLowerCase()}`,
    ],
  };
});
