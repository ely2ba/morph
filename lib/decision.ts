...
export const assumptionMeta: Record<AssumptionId, { label: string; storageUnit: string; displayUnit: string; min: number; max: number; step: number; example: number | string; description: string; validValues?: MachineType[] }> = {
  ...
  machine_type: { label: 'Machine type', storageUnit: 'enum', displayUnit: 'type', min: 0, max: 0, step: 0, example: 'integrated', validValues: ['freestanding', 'integrated'], description: 'Freestanding or integrated installation.' },
};
...
export function findNearestRelaxations(config: DecisionConfig, hiddenIds: string[] = []) {
  const assumptions = config.assumptions;
  const candidates: Relaxation[] = [];
  const specs: Array<{ id: NumericAssumptionId; requirementId: RequirementId; direction: 'increase' | 'decrease'; getter: (p: Product) => number }> = [
    { id: 'opening_depth_cm', requirementId: 'fits_opening', direction: 'increase', getter: (p) => p.installedDepthCm },
    { id: 'opening_width_cm', requirementId: 'fits_opening', direction: 'increase', getter: (p) => p.widthCm },
    { id: 'opening_height_cm', requirementId: 'fits_opening', direction: 'increase', getter: (p) => p.heightCm },
    { id: 'delivery_within_days', requirementId: 'delivery_within_days', direction: 'increase', getter: (p) => p.deliveryDays },
    { id: 'minimum_capacity_kg', requirementId: 'minimum_capacity_kg', direction: 'decrease', getter: (p) => p.capacityKg },
    { id: 'maximum_purchase_price', requirementId: 'maximum_purchase_price', direction: 'increase', getter: (p) => p.price },
  ];

  for (const [stableOrder, spec] of specs.entries()) {
    const current = assumptions[spec.id];
    if (current == null || !config.requirements.some((requirement) => requirement.id === spec.requirementId)) continue;
    const thresholds = [...new Set(catalog.map(spec.getter).filter((threshold) => spec.direction === 'increase' ? threshold > current : threshold < current))]
      .sort((a, b) => spec.direction === 'increase' ? a - b : b - a);

    for (const threshold of thresholds) {
      const nextRequirements = config.requirements.map((requirement) =>
        requirement.id === spec.id && requirement.value != null ? { ...requirement, value: threshold } : requirement,
      );
  ...
  return candidates
    .sort((a, b) => a.proportional - b.proportional || b.admitted - a.admitted || a.stableOrder - b.stableOrder)
    .slice(0, 3);
}
...
export type CalculationBreakdown = {
  ...
  steps: Array<{ id: string; label: string; expression: string; value: number; formattedValue: string }>;
  estimateBasis: string;
  exclusions: string[];
};

const number = (amount: number, maximumFractionDigits = 4) => amount.toLocaleString('en-GB', { maximumFractionDigits });
const money = (amount: number, fixed = true) => fixed
...
function formattedMetricResult(metricId: DerivedMetricId, result: number) {
  if (['ownership_cost', 'energy_cost', 'water_cost', 'annual_running_cost'].includes(metricId)) return money(result);
  if (metricId === 'running_cost_per_cycle') return `${(result * 100).toFixed(1)}p per wash`;
  if (metricId === 'narrowest_clearance_cm') return `${number(result, 1)} cm`;
  if (metricId === 'delivery_slack_days') return `${number(result, 0)} ${plural(result, 'day')}`;
  if (metricId === 'annual_energy_kwh') return `${number(result, 2)} kWh/year`;
  return `${number(result, 0)} L/year`;
}

export function explainMetric(evaluation: Evaluation, metricId: DerivedMetricId, config: DecisionConfig): CalculationBreakdown {
  const assumptions = config.assumptions;
  ...
  const electricityRate = value(assumptions, 'electricity_price_pence_per_kwh') / 100;
  ...
  const step = (id: string, label: string, expression: string, stepValue: number, formattedValue: string) => ({ id, label, expression, value: stepValue, formattedValue });
  const input = (id: string, label: string, display: string) => ({ id, label, display });
  const fact = (id: string, label: string, display: string) => ({ id, label, display });
  ...
  if (['ownership_cost', 'energy_cost', 'water_cost'].includes(metricId)) {
    shopperInputs = [
      input('cycles_per_week', 'Washes each week', `${number(cyclesPerWeek)} ${plural(cyclesPerWeek, 'wash', 'washes')}`),
      input('ownership_years', 'Ownership period', `${number(ownershipYears)} ${plural(ownershipYears, 'year')}`),
    ];
    steps.push(step('total_cycles', 'Total cycles', `${number(cyclesPerWeek)} ${plural(cyclesPerWeek, 'wash', 'washes')}/week × 52 weeks × ${number(ownershipYears)} ${plural(ownershipYears, 'year')} = ${number(totalCycles)} cycles`, totalCycles, `${number(totalCycles)} cycles`));
  }

  if (metricId === 'ownership_cost' || metricId === 'energy_cost') {
    shopperInputs.push(input('electricity_rate', 'Electricity price', `${number(value(assumptions, 'electricity_price_pence_per_kwh'))}p/kWh`));
    productFacts.push(fact('energy_per_100', 'Eco 40–60 energy', `${number(product.energyKwhPer100)} kWh/100 cycles`));
    steps.push(
      step('energy_use', 'Electricity use', `${number(product.energyKwhPer100)} kWh/100 cycles × ${number(totalCycles)} cycles = ${number(energyOverOwnership)} kWh`, energyOverOwnership, `${number(energyOverOwnership)} kWh`),
      step('energy_cost', 'Electricity cost', `${number(energyOverOwnership)} kWh × ${money(electricityRate, false)} = ${money(energyCost)}`, energyCost, money(energyCost)),
    );
  }

  if (metricId === 'ownership_cost' || metricId === 'water_cost') {
    shopperInputs.push(input('water_rate', 'Water price', `${number(value(assumptions, 'water_price_pence_per_litre'))}p/litre`));
    productFacts.push(fact('water_per_cycle', 'Eco 40–60 water', `${number(product.waterLitresPerCycle)} L/cycle`));
    steps.push(
      step('water_use', 'Water use', `${number(product.waterLitresPerCycle)} L/cycle × ${number(totalCycles)} cycles = ${number(waterOverOwnership)} L`, waterOverOwnership, `${number(waterOverOwnership)} L`),
      step('water_cost', 'Water cost', `${number(waterOverOwnership)} L × ${money(waterRate, false)} = ${money(waterCost)}`, waterCost, money(waterCost)),
    );
  }

  if (metricId === 'ownership_cost') {
    productFacts.unshift(fact('purchase_price', 'Purchase price', money(product.price, false)));
    steps.push(step('ownership_total', 'Final addition', `${money(product.price, false)} + ${money(energyCost)} + ${money(waterCost)} = ${money(evaluation.metrics.ownership_cost)}`, evaluation.metrics.ownership_cost, money(evaluation.metrics.ownership_cost)));
  } else if (metricId === 'annual_running_cost' || metricId === 'running_cost_per_cycle') {
    shopperInputs = [
      ...(metricId === 'annual_running_cost' ? [input('cycles_per_week', 'Washes each week', `${number(cyclesPerWeek)} ${plural(cyclesPerWeek, 'wash', 'washes')}`)] : []),
      input('electricity_rate', 'Electricity price', `${number(value(assumptions, 'electricity_price_pence_per_kwh'))}p/kWh`),
      input('water_rate', 'Water price', `${number(value(assumptions, 'water_price_pence_per_litre'))}p/litre`),
    ];
    productFacts = [
      fact('energy_per_100', 'Eco 40–60 energy', `${number(product.energyKwhPer100)} kWh/100 cycles`),
      fact('water_per_cycle', 'Eco 40–60 water', `${number(product.waterLitresPerCycle)} L/cycle`),
    ];
    steps = [
      ...(metricId === 'annual_running_cost' ? [step('annual_cycles', 'Annual cycles', `${number(cyclesPerWeek)} ${plural(cyclesPerWeek, 'wash', 'washes')}/week × 52 weeks = ${number(annualCycles)} cycles/year`, annualCycles, `${number(annualCycles)} cycles/year`)] : []),
      step('energy_per_cycle', 'Electricity per wash', `${number(product.energyKwhPer100)} kWh/100 cycles = ${number(energyPerCycle)} kWh/cycle`, energyPerCycle, `${number(energyPerCycle)} kWh/cycle`),
      step('energy_cost_per_cycle', 'Electricity cost per wash', `${number(energyPerCycle)} kWh × ${money(electricityRate, false)} = ${money(energyCostPerCycle, false)}`, energyCostPerCycle, money(energyCostPerCycle, false)),
      step('water_cost_per_cycle', 'Water cost per wash', `${number(product.waterLitresPerCycle)} L × ${money(waterRate, false)} = ${money(waterCostPerCycle, false)}`, waterCostPerCycle, money(waterCostPerCycle, false)),
      step('running_cost_per_cycle', 'Total per wash', `${money(energyCostPerCycle, false)} + ${money(waterCostPerCycle, false)} = ${money(costPerCycle, false)}`, costPerCycle, `${(costPerCycle * 100).toFixed(1)}p`),
      ...(metricId === 'annual_running_cost' ? [step('annual_running_cost', 'Annual running cost', `${money(costPerCycle, false)} × ${number(annualCycles)} cycles = ${money(evaluation.metrics.annual_running_cost)}`, evaluation.metrics.annual_running_cost, money(evaluation.metrics.annual_running_cost))] : []),
    ];
  } else if (metricId === 'annual_energy_kwh') {
    shopperInputs = [input('cycles_per_week', 'Washes each week', `${number(cyclesPerWeek)} ${plural(cyclesPerWeek, 'wash', 'washes')}`)];
    productFacts = [fact('energy_per_100', 'Eco 40–60 energy', `${number(product.energyKwhPer100)} kWh/100 cycles`)];
    steps = [
      step('annual_cycles', 'Annual cycles', `${number(cyclesPerWeek)} ${plural(cyclesPerWeek, 'wash', 'washes')}/week × 52 weeks = ${number(annualCycles)} cycles/year`, annualCycles, `${number(annualCycles)} cycles/year`),
      step('annual_energy', 'Annual electricity use', `${number(product.energyKwhPer100)} kWh/100 cycles × ${number(annualCycles)} cycles = ${number(annualEnergy)} kWh/year`, annualEnergy, `${number(annualEnergy)} kWh/year`),
    ];
  } else if (metricId === 'annual_water_litres') {
    shopperInputs = [input('cycles_per_week', 'Washes each week', `${number(cyclesPerWeek)} ${plural(cyclesPerWeek, 'wash', 'washes')}`)];
    productFacts = [fact('water_per_cycle', 'Eco 40–60 water', `${number(product.waterLitresPerCycle)} L/cycle`)];
    steps = [
      step('annual_cycles', 'Annual cycles', `${number(cyclesPerWeek)} ${plural(cyclesPerWeek, 'wash', 'washes')}/week × 52 weeks = ${number(annualCycles)} cycles/year`, annualCycles, `${number(annualCycles)} cycles/year`),
      step('annual_water', 'Annual water use', `${number(product.waterLitresPerCycle)} L/cycle × ${number(annualCycles)} cycles = ${number(annualWater)} L/year`, annualWater, `${number(annualWater)} L/year`),
    ];
  } else if (metricId === 'narrowest_clearance_cm') {
    const dimensions = [
      ['width', value(assumptions, 'opening_width_cm'), product.widthCm, evaluation.clearances.width],
      ['depth', value(assumptions, 'opening_depth_cm'), product.installedDepthCm, evaluation.clearances.depth],
      ['height', value(assumptions, 'opening_height_cm'), product.heightCm, evaluation.clearances.height],
    ] as const;
    const tightest = dimensions.reduce((best, current) => current[3] < best[3] ? current : best);
    shopperInputs = dimensions.map(([dimension, opening]) => input(`opening_${dimension}`, `Opening ${dimension}`, `${number(opening, 1)} cm`));
    productFacts = dimensions.map(([dimension, , installed]) => fact(`installed_${dimension}`, `Required installed ${dimension}`, `${number(installed, 1)} cm`));
    steps = [
      ...dimensions.map(([dimension, opening, installed, clearance]) => step(`${dimension}_clearance`, `${dimension[0].toUpperCase()}${dimension.slice(1)} clearance`, `${number(opening, 1)} cm − ${number(installed, 1)} cm = ${number(clearance, 1)} cm`, clearance, `${number(clearance, 1)} cm`)),
      step('tightest_clearance', 'Tightest dimension', `${tightest[0][0].toUpperCase()}${tightest[0].slice(1)} is the tightest at ${number(tightest[3], 1)} cm`, tightest[3], `${number(tightest[3], 1)} cm`),
    ];
    ...
    shopperInputs = [input('delivery_within_days', 'Latest acceptable delivery', `${number(allowedDays, 0)} ${plural(allowedDays, 'day')}`)];
    productFacts = [fact('delivery_days', 'Quoted delivery', `${number(product.deliveryDays, 0)} ${plural(product.deliveryDays, 'day')}`)];
    steps = [step('delivery_slack', 'Delivery slack', `${number(allowedDays, 0)} allowed ${plural(allowedDays, 'day')} − ${number(product.deliveryDays, 0)} delivery ${plural(product.deliveryDays, 'day')} = ${number(slack, 0)} ${plural(slack, 'day')}`, slack, `${number(slack, 0)} ${plural(slack, 'day')}`)];
  }

  let estimateBasis = 'Calculated from the product’s Eco 40–60 specification and your current usage and utility inputs.';
  let exclusions = ['Non-Eco programmes', 'Tariff standing charges', 'Changes to future utility prices'];
  if (metricId === 'ownership_cost') {
    estimateBasis = 'Calculated from the current purchase price, the product’s Eco 40–60 specification, and your usage and utility inputs.';
    exclusions = ['Detergent', 'Repairs and maintenance', 'Finance costs', 'Resale value'];
  } else if (metricId === 'annual_energy_kwh') {
    estimateBasis = 'Calculated from Eco 40–60 energy use and your weekly wash estimate.';
    exclusions = ['Other programmes and temperatures', 'Standby energy', 'Changes in household usage'];
  } else if (metricId === 'annual_water_litres') {
    estimateBasis = 'Calculated from Eco 40–60 water use and your weekly wash estimate.';
    exclusions = ['Other programmes and load sizes', 'Installation leaks', 'Changes in household usage'];
  } else if (metricId === 'narrowest_clearance_cm') {
    estimateBasis = 'Calculated from your measured opening minus the retailer’s required installed dimensions, including stated clearances.';
    exclusions = ['Door-opening space', 'Uneven floors or walls', 'Installer tolerances beyond the stated specification'];
  } else if (metricId === 'delivery_slack_days') {
    estimateBasis = 'Calculated from your latest acceptable delivery day minus the retailer’s current quoted delivery offset.';
    exclusions = ['Carrier delays', 'Installation appointment availability', 'Delivery-date changes after ordering'];
  }
  ...
  return {
    ...
    formattedResult: formattedMetricResult(metricId, evaluation.metrics[metricId]),
    shopperInputs,
    productFacts,
    steps,
    estimateBasis,
